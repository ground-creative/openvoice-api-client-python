...
class TestClientAsync(unittest.TestCase):
    ...
    @patch('os.path.getsize', return_value=12345)  # Mock file size to avoid FileNotFoundError
    @patch('requests.post')
    async def test_generate_audio_bytes_as_base64(self, mock_post, mock_getsize, mock_open):
        # Mock response data for successful response
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
    ...
    async def test_change_voice(self, mock_post, mock_open):
        ...
        mock_response.content = asyncio.BytesIO(b'changed_audio_bytes_content')  # Mock audio bytes content
