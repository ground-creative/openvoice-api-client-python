...
class TestClientAsync(unittest.TestCase):
    async def setUpAsync(self):
        ...
        self.client = OpenVoiceApiClientAsync()
    ...
    @patch(
        "os.path.getsize", return_value=12345
    )  # Mock file size to avoid FileNotFoundError
    @patch("requests.post")
    async def test_generate_audio_bytes(self, mock_post, mock_getsize, mock_open):
        ...
        )  # Verify that output_file path is returned
        mock_open.assert_called_once_with(
            output_file, "wb"
        )  # Ensure open was called with output_file
        mock_open().write.assert_called_once_with(
            b"audio_bytes_content"
        )  # Ensure write was called with audio content
    ...
    @patch("requests.post")
    async def test_generate_audio_bytes_as_bytes(
        self, mock_post, mock_getsize, mock_open
    ):
        # Mock response data for successful response
        mock_response = MagicMock()
        mock_response.status_code = 200
        ...
        )  # Verify that audio_bytes is of type bytes
    ...
    @patch("requests.post")
    async def test_generate_audio_base64(self, mock_post, mock_getsize, mock_open):
        # Mock response data for successful response
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
            "result": {
                "data": {
                    "audio_data": base64.b64encode(b"audio_bytes_content").decode(
                        "utf-8"
                    )
                }
            }
        ...
        called_args = mock_open().write.call_args[0][
            0
        ]  # Get the arguments written to mock_open
    ...
    @patch("requests.post")
    async def test_generate_audio_bytes_as_base64(
        self, mock_post, mock_getsize, mock_open
    ):
        # Mock response data for successful response
        mock_response = MagicMock()
        ...
        audio_base64, status_code, message = await self.client.generate_audio(
            version="v2",
            model="en",
            input="Hello, this is a test",
            voice="kaiwen",
        ...
        # Assertions
        self.assertEqual(status_code, 200)
        self.assertEqual(type(audio_base64), str, "audio_bytes is not of type str")

        try:
            audio_bytes = base64.b64decode(audio_base64)
        except Exception as e:
            self.fail(f"audio_base64 is not a valid base64 encoded string: {e}")
        self.assertEqual(
            type(audio_bytes), bytes, "Converted audio_bytes is not of type bytes"
        )
    ...
    async def test_change_voice(self, mock_post, mock_open):
        # Mock response data for successful response
        mock_response = MagicMock()
        mock_response.status = 200
        mock_response.content = asyncio.BytesIO(
            b"changed_audio_bytes_content"
        )  # Mock audio bytes content
        ...
        audio_bytes, status_code, message = await self.client.change_voice(
            audio_data=encoded_audio_data, voice="elon", response_format="bytes"
        )
    ...
    @patch("aiofiles.open", new_callable=mock_open)
    @patch("aiohttp.ClientSession.post")
    async def test_change_voice_encode_param(self, mock_post, mock_open):
        # Mock response data for successful response
        mock_response = MagicMock()
        mock_response.status = 200
        mock_response.content = asyncio.BytesIO(
    ...
    @patch("aiofiles.open", new_callable=mock_open)
    @patch("aiohttp.ClientSession.post")
    async def test_change_voice_audio_file_param(self, mock_post, mock_open):
        # Mock response data for successful response
        mock_response = MagicMock()
        mock_response.status = 200
        mock_response.content = asyncio.BytesIO(
        ...
        # Call the async method
        audio_bytes, status_code, message = await self.client.change_voice(
            audio_file=audio_file, voice="elon", encode=True, response_format="bytes"
        )
    ...
    async def test_generate_audio_bad_request(self):
        ...
        mock_response.json.return_value = {"data": {"message": "Bad Request"}}
